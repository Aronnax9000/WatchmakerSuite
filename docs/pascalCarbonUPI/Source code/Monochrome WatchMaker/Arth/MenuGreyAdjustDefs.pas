...
{SetItemState(AM, 0, (TheMode <> Drifting) AND (TheMode <> Highlighting) AND (TheMode <> Albuming));  }
		...
		{Load to SlideBox}
		SetItemState(kWatchMenuIndexFile, kWatchMenuFileSaveBiomorph, Special > 0); {Save Biomorph}
		...
		SetItemState(kWatchMenuIndexEdit, kWatchMenuEditClear, (Special > 0) AND (TheMode = Albuming));
		SetItemState(kWatchMenuIndexEdit, kWatchMenuEditCopy, (Special > 0));
		CheckPastable;
		...
		SetItemState(kWatchMenuIndexOperation, kWatchMenuOperationBreed, Special > 0);  {Breed}
		SetItemState(kWatchMenuIndexOperation, kWatchMenuOperationDrift, Special > 0);  {Drift}
		SetItemState(kWatchMenuIndexOperation, kWatchMenuOperationEngineering, Special > 0);  {3 Engineering}
		{4 Hopeful Monster}
		...
		SetItemState(kWatchMenuIndexPedigree, kWatchMenuPedigreeDisplayPedigree, Special > 0);
