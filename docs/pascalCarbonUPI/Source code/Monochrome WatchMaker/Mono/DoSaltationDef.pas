...
interface
	...
	procedure DoSaltation(boxes: HIBoxesPtr);
...
implementation
	...
	procedure DoSaltation(boxes: HIBoxesPtr);
		...
		begin
			...
			WITH special^.denizen^ DO {bomb 5, range check failed, here after killing top Adam}
				begin
					IF Mut[1] THEN
						begin
							SegNoGene := randint(6);
			...
			Develop(special, boxes^.MidBox^.MidPoint);
