...
public class ArthromorphsEngineer extends JFrame {
    ...
    public ArthromorphsEngineer(AppData appData) {
        ...
        final ArthromorphAllowedMutations muts = (ArthromorphAllowedMutations) appData
                .getMorphConfig().getMutagen().getAllowedMutations();
        this.addComponentListener(new ComponentAdapter() {
            public void componentShown(ComponentEvent e) {
                ...
                focusFirst.setSelected(
                        muts.focusOfAttention == Concentration.FirstSegmentOnly);
                focusLast.setSelected(
                        muts.focusOfAttention == Concentration.LastSegmentOnly);
                focusNone.setSelected(
                        muts.focusOfAttention == Concentration.AnySegment);
                ...
                mutPressurePlus.setSelected(
                        muts.mutationPressure == Pressure.Positive);
                mutPressureZero
                        .setSelected(muts.mutationPressure == Pressure.Zero);
                mutPressureMinus.setSelected(
                        muts.mutationPressure == Pressure.Negative);
            }
        });
    ...
    private void cautionAlert(String message) {
        JOptionPane.showMessageDialog(this, message,
                "Allowed Mutations Warning", JOptionPane.WARNING_MESSAGE,
                new ImageIcon(((BufferedImage) appData.getClassicImageLoader()
                        .getPicture("AlertCautionAlert_cicn_00002_32x32"))));

    }

    /**
     * See Arthromorphs/Engineering_Window.pas, PROCEDURE D_Engineering_Window,
     * lines 442-643
     */
    private void handleOKButton() {
        ...
        if (angleMut.isSelected() || heightMut.isSelected()
                || lengthMut.isSelected()) {
            dearthOfAtomMuts = false;
            dupDeleteOnly = false;
        }
        ...
        if (animalTrunkMut.isSelected() || animalLegsMut.isSelected()
                || animalClawsMut.isSelected() || sectionClawsMut.isSelected()
                || segmentClawsMut.isSelected() || clawsMut.isSelected())
            dearthOfBodyMuts = false;

        boolean animalOrClawsOnly = true;

        if (sectionTrunkMut.isSelected() || sectionLegsMut.isSelected()
                || segmentTrunkMut.isSelected() || segmentLegsMut.isSelected()
                || legsMut.isSelected()) {
            dearthOfBodyMuts = false;
        ...
        if (dearthOfAtomMuts) {
            ...
            String errorMessage = Messages.getMessages()
                    .getString("Arth_DearthOfAtomMuts");
            cautionAlert(errorMessage);
        }
        if (dearthOfBodyMuts) {
            agreeToExit = false;
            String errorMessage = Messages.getMessages()
                    .getString("Arth_DearthOfBodyMuts");
            cautionAlert(errorMessage);
        ...
        if (agreeToExit) {
            ArthromorphAllowedMutations muts = (ArthromorphAllowedMutations) appData
                    .getMorphConfig().getMutagen().getAllowedMutations();
            ...
            muts.focusOfAttention = focusFirst.isSelected()
                    ? Concentration.FirstSegmentOnly
                    : focusLast.isSelected() ? Concentration.LastSegmentOnly
                            : focusNone.isSelected() ? Concentration.AnySegment
                                    : null;
            muts.heightMut = heightMut.isSelected();
            muts.legsMut = legsMut.isSelected();
            muts.mutationPressure = mutPressurePlus.isSelected()
                    ? Pressure.Positive
                    : mutPressureZero.isSelected() ? Pressure.Zero
                            : mutPressureMinus.isSelected() ? Pressure.Negative
                                    : null;
