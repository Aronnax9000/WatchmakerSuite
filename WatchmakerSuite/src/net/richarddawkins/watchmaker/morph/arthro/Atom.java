...
public class Atom implements Cloneable {
  ...
  public Atom(AtomKind kind, double height, double width, double angle, Atom nextLikeMe,
      Atom firstBelowMe) {
    this.kind = kind;
    this.height = height;
    this.width = width;
   ...
   * <ul>
   * <li>also used in an AnimalTrunk to store the number of atoms in the animal.</li>
    ...
    if (parent != null)
      if (parent.kind == AtomKind.SectionTrunk || parent.kind == AtomKind.SegmentTrunk
        ...
        if (parent.nextLikeMe != null) {
          ...
          parent.nextLikeMe.nextLikeMe = null; // So Kill won't get the rest of chain
          parent.nextLikeMe.kill(); // won't be killing last one, since parent qualifies as one
          parent.nextLikeMe = chain;
          doDelete = true;
        } else // Try to delete FirstBelow
        if (parent.firstBelowMe != null) { // we know FirstBelow exists
          chain = parent.firstBelowMe.nextLikeMe; // Atom after one we will delete
  ...
  public void draw(Graphics2D g2, double[] params, int x, int y, int xCenter, int ySeg)
    ...
    // The original Pascal implementation, grabbed gradient from AnimalTrunk as it went by,
    // storing the value in a static (global) variable.
    // the present implementation stores gradient separately, in the genome.
    gradientFactor = genome.getGradient();

    if (kind == AtomKind.AnimalTrunk) {
      if (gradientFactor > 1000)
        throw new ArthromorphGradientExceeds1000Exception(
            "Arthromorph gradient is " + gradientFactor
                + " which exceeds 1000. The original code would beep here and continue.");
    }

    int offset = paramOffset[kind.ordinal()]; // where in params to begin, see InitBoneYard
    params[offset] *= height; // fold in this atom's params
    params[offset + 1] *= width;
    params[offset + 2] *= angle;// Must be a real number, even if not used in this Atom
    ...
    if (kind == AtomKind.SectionTrunk) {
      ...
      x += (int) Math.round(params[1] / 2.0); // joint starts at the side of the segment
      ...
      oldY = y;
      double ang = params[5];
      x += (int) Math.round(jointscale * params[4] * Math.cos(ang)); // line end point
                                                                     // width*sine(angle)
      y += (int) Math.round(jointscale * params[4] * Math.sin(ang)); // line end point
      int thick = 1 + (int) Math.floor(params[3]); // 1 is minimum thickness
    ...
    if (kind == AtomKind.Claw) {
      ...
      if (firstBelowMe != null)
        firstBelowMe.draw(g2, params, x, y, xCenter, ySeg); // build on my cumulative numbers
      ...
      if (nextLikeMe != null) {
        ...
        if (kind == AtomKind.AnimalJoint || kind == AtomKind.SectionJoint
            || kind == AtomKind.SegmentJoint)
          nextLikeMe.draw(g2, params, x, y, xCenter, ySeg); // build on me
        else if (kind != AtomKind.AnimalTrunk)
          nextLikeMe.draw(g2, myPars, x, y, xCenter, ySeg); // build on my parent's numbers
      }
    }
  }

  /**
   * <h2>Original documentation</h2> Draw a claw, note that we don't [refer to a specific atom] at
   * all. Param info is already folded in
   * 
   * @param params
   *          array of 9 doubles
   * @param x
  ...
  void drawClaw(Graphics2D g2, double[] params, int x, int y, int xCenter) {
    ...
    x = (int) Math.round(x + params[7] * Math.sin(ang)); // line end point width*sine(angle)
    y = (int) Math.round(y + params[7] * Math.cos(ang)); // line end point
    // Used floor instead of Pascal trunc. Maybe not so good. Should be fine if no negative numbers
    // ABC
  ...
  void drawLine(Graphics2D g2, int x, int y, int endX, int endY, int thick) {
    ...
    genome.expandPoles(endY, endY, endX, endX);
    if (g2 != null) {
  ...
  public void drawOval(Graphics2D g2, int x, int y, int width, int height) {
    ...
    genome.expandPoles(rect.top, rect.bottom, rect.left, rect.right);
    if (g2 != null) {
