...
public class SwingMorphDrawer implements MorphDrawer {
	...
	public void draw(BoxedMorph boxedMorph, Object graphicsContext, Dim d) {
		...
		logger.fine("Scale: "+ scale);
//		picDrawer.drawPic(g2, phenotype);
		BufferedImage bufferedImage = picDrawer.getBufferedImage(phenotype, scale);
		Point position = boxedMorph.getPosition(d);
		
		g2.translate(
				position.h - bufferedImage.getWidth() / 2,
				position.v -bufferedImage.getHeight() / 2
				);
